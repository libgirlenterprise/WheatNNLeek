...
impl PassiveDevice for SynapseS0S1 {}
...
impl ConsecutivePassiveDevice for SynapseS0S1 {
    fn respond(&mut self) {
...
impl SynapseS0S1 {
          ...
          A: 'static + PassiveAcceptor<FwdPostS1>,
    {
        ...
        SynapseS0S1::new_with_passive(value, device1, device2)
    }
          ...
          A: 'static + ActiveAcceptor<FwdPostS1>,
    {
        let device1 = p1.lock().unwrap().device_by_id(n1).clone();
        let device2 = p2.lock().unwrap().device_by_id(n2).clone();
        SynapseS0S1::new_with_active(value, device1, device2)
    }
